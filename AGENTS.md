...
Cases index correction, confirmed 2026-08-21: the unified dark showcase must expose all three current cases at once on desktop — one dominant case on the left and two complete secondary cases stacked on the right. Titles, metrics, and full-card links stay visible without scrolling inside the board. Intrinsic image dimensions must never expand grid items or push case copy outside the clipped board.

Selected case-detail direction, confirmed 2026-08-21: use the approved “Swiss Editorial Proof” mockup. Each case detail opens on a tactile pearl/ice field with a restrained Cormorant Garamond title, three asymmetrical vertical proof frames, and one continuous cyan outcome rail. Follow with one unified deep-navy media board and a light three-part Context / System / Result narrative. Adapt imagery and proof language to each case; never reuse content-factory visuals as generic decoration for automation or recruiting.

Case-detail hero correction, confirmed 2026-08-21: desktop hero must fit inside one viewport and use `public/assets/sweezy/swiss-atlas-bg.webp` as its real tactile pearl/topographic background. Size vertical proof frames from both `vw` and `svh`, position their images as absolute layers, and never let a portrait asset's intrinsic height expand the hero beyond the screen.
